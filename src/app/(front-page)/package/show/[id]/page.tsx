...
const BookingPackagePage = () => {
 ...
 const  [packageSingle, setPackageSingle] = React.useState<any>(null)
  ...
  useEffect(() => {
    ...
    if (authState._is_auth && authState._auth && authState._avaibility && id) {
      handlegetPackage()
    }
    // console.log('authState', authState._auth)
    // getPackage("/package")
  }, [authState, id])
  ...
  const handlegetPackage = async () => {
    try {
      const res = await api.get(`/package/show/${id}`)
      setPackageSingle(res.data)
      // console.log(res)
    } catch (error) {
      router.push('/')
      toast.error('url not valid')
  ...
  const procesedTransaction = async () => {
    try {
      if (!packageSingle  || selectedPayment !== null) {
        const payload = {
          payment_method : selectedPayment,
        }
        const res = await procesedPackage(`/payment/${packageSingle?.id}`, payload)
      ...
      <div className='container pt-20 pb-10 mt-20 flex flex-col lg:flex-row gap-7'>
        <div className='p-5 border border-gray-200 rounded-lg w-12/12 lg:w-9/12'>
...

          {
            ...
            : <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-3'>
              <PackageCard select={() => false}  data={packageSingle} hiddenSelect={true}/>
                {/* {
